...
def process_viirs_adp_saai_smoke(ds):
    """Process VIIRS ADP smoke data to extract SAAI smoke values.
    Args:
        ds (xarray.Dataset): Xarray dataset containing VIIRS ADP data.
    Returns:
        saai_smoke (numpy.ndarray): Processed SAAI smoke values.
    """
        ...
        with fs.open(file, mode="rb") as remote_file:
            with xr.open_dataset(remote_file, engine="h5netcdf") as ds:
                ...
                grid_array = project_data_to_grid(
                    grid_array, geotransform, lon, lat, saai_smoke, grid_size
                )
